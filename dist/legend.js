...
System.register(["angular", "app/core/utils/kbn", "jquery", "jquery.flot", "jquery.flot.time", "./lib/perfect-scrollbar.min"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      angular.module("grafana.directives").directive("scatterplotLegend", function (popoverSrv, $timeout) {
        return {
          link: function link(scope, elem) {
            ...
            function render() {
              ...
              for (i = 0; i < seriesList.length; i++) {
                ...
                html += '<a class="scatterplot-legend-alias" style="float:none;">' + seriesData.label + "</a>";

                if (showValues && tableLayout) {
                  var value = series.stats[ctrl.panel.valueName];
                  if (panel.legend.values) {
                    html += '<div class="scatterplot-legend-value">' + ctrl.formatValue(value) + "</div>";
                  }
                  if (total) {
            ...
            function addScrollbar() {
              ...
              if (!legendScrollbar) {
                legendScrollbar = new PerfectScrollbar(".scatterplot-legend", scrollbarOptions);
              } else {
