...
import $ from 'jquery';
import 'jquery.flot';
import 'jquery.flot.scatter';

export default function link(scope, elem, attrs, ctrl) {
  var data, panel;
  elem = elem.find('.scatterchart-panel__chart');
  ...
  function addscatterChart() {
    ...
    if (panel.scatterType === 'donut') {
      options.series.scatter.innerRadius = 0.5;
    }
    ...
    plotCanvas.bind("plothover", function (event, pos, item) {
      ...
      body = '<div class="scatterchart-tooltip-small"><div class="scatterchart-tooltip-time">';
      body += '<div class="scatterchart-tooltip-value">' + item.series.label + ': ' + formatted;
      body += " (" + percent + "%)" + '</div>';
      body += "</div></div>";
